...
- [todo] bench-elo: Maintain a running Elo rating per policy at `bench/elo.csv` (cols: `timestamp,policy,games,wins,losses,elo`). Each call to `run_continuous.py` updates Elo using the new matchup results (K=32, R0=1500). Acceptance: after 50 rounds of greedy-vs-random benching, `greedy`'s Elo is at least 200 above `random`'s. Add a unit test that simulates two rounds of fake results and asserts the Elo update formula matches a hand-computed value.
