...
- [todo] competitor-self-validate: Add `scripts/validate_competitor.py` that constructs two `VgcAiCompetitor()` instances and runs `vgc2.competition.match.Match` between them for 10 doubles battles (`n_active=2`), asserting that the match completes without error and reports a winner per battle. Used as a competition-submission smoke test. Acceptance: `uv run python scripts/validate_competitor.py` exits 0 in under 60 seconds; prints final win count. No bench evidence required.
