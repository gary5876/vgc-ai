# Reference Study: VGC Battle Agents (Archived)

> **Archived — superseded.** This study covered the Pokemon Showdown / poke-env
> ecosystem on the assumption we would target NeurIPS PokéAgent or a similar
> Showdown-based competition. The project pivoted on 2026-05-11 to the
> **IEEE VGC AI Competition 2026** (CoG 2026, Madrid), which uses an entirely
> different framework (`pokemon-vgc-engine` / `vgc2` by Simão Reis).
>
> The conclusions about VGC-Bench, EliteFurretAI, and PokéChamp **do not transfer**.
> See the current `REFERENCE_STUDY.md` and `CLAUDE.md` for the active plan.
> Kept in version control as historical context for the pivot decision.
