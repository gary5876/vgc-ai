...
Currently aliases `GreedyBattlePolicy` as the default — submission-viable: ~0.04s
per battle in doubles, ~90% win rate vs `RandomBattlePolicy`. `TreeSearchBattlePolicy`
is stronger (100% vs random in our test) but measures ~170s per battle / ~11s per
...
Also exposes `GreedyWithSwitchBattlePolicy`, a thin layer on top of greedy that
swaps a low-HP active for a bench mon with a STAB super-effective answer to one
of the opponent's actives.
...
class GreedyWithSwitchBattlePolicy(GreedyBattlePolicy):  # type: ignore[misc]  # vgc2 is untyped; GreedyBattlePolicy resolves as Any under --strict
    """Greedy + one switching rule.
    ...
    damaging move whose type is super-effective against any of the opponent's
    actives, switch to that bench mon. Otherwise fall back to greedy.
...
__all__ = ["GreedyWithSwitchBattlePolicy", "VgcAiBattlePolicy"]
